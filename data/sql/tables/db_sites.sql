...
CREATE OR REPLACE VIEW sites_i18n AS SELECT sa.id, sa.culture, sa.name, sa.search_name, sa.description, sa.remarks, sa.pedestrian_access, sa.way_back, sa.site_history FROM app_sites_i18n_archives sa WHERE sa.is_latest_version;
...
(
    INSERT INTO app_sites_i18n_archives (id, culture, name, search_name, description, remarks, pedestrian_access, way_back, site_history, is_latest_version) VALUES (NEW.id, NEW.culture, NEW.name, NEW.search_name, NEW.description, NEW.remarks, NEW.pedestrian_access, NEW.way_back, NEW.site_history, true)
);
