...
CREATE OR REPLACE VIEW images AS SELECT sa.oid, sa.id, sa.lon, sa.lat, sa.elevation, sa.module, sa.is_protected, sa.redirects_to, sa.geom, sa.geom_wkt, sa.filename, sa.v4_id, sa.v4_app, sa.categories, sa.activities, sa.author, sa.date_time, sa.camera_name, sa.exposure_time, sa.fnumber, sa.iso_speed, sa.focal_length FROM app_images_archives sa WHERE sa.is_latest_version; 
...
(
    INSERT INTO app_images_archives (id, module, is_protected, redirects_to, geom, geom_wkt, lon, lat, elevation, filename, categories, activities, author, date_time, camera_name, exposure_time, fnumber, iso_speed, focal_length, v4_id, v4_app, is_latest_version) VALUES (NEW.id, 'images', NEW.is_protected, NEW.redirects_to, NEW.geom, NEW.geom_wkt, NEW.lon, NEW.lat, NEW.elevation, NEW.filename, NEW.categories, NEW.activities, NEW.author, NEW.date_time, NEW.camera_name, NEW.exposure_time, NEW.fnumber, NEW.iso_speed, NEW.focal_length, NEW.v4_id, NEW.v4_app, true)
);
