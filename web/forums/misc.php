...
else if ($action == 'markread')
{
	...
	redirect(get_home_url(), $lang_misc['Mark read redirect']);
}
...
else if ($action == 'markforumread')
{
	...
	redirect(get_home_url(), $lang_misc['Mark forum read redirect']);
}
...
else if (isset($_GET['email']))
{
	...
	if (isset($_POST['form_sent']))
	{
		...
		else if ($_POST['req_email'] != '' && $pun_user['username'] == 'Guest')
                        $pun_user['email']=$_POST['req_email'];
                
                
                if(!$pun_user['email'])
                        message($lang_common['E-mail']);
                
                if (($pun_config['o_guest_post_captchabox'] == '1') && !isset($_POST['preview']) && $pun_user['is_guest']) {
                    $picture=$_SESSION["captchabox"];
                    $a=$picture->check();
                    if (CHECK_KO == $a ) {
				...
				<fieldset>
					...
					<div class="infldset txtarea">
						...
						<input type="hidden" name="redirect_url" value="<?php echo $redirect_url ?>" />
                                                <?php if($pun_user['email'] == 'Guest'){ ?>
						<label><strong><?php echo $lang_common['E-mail'] ?></strong><br />
                        ...
                        <?php if (($pun_config['o_guest_post_captchabox'] == '1') and ($pun_user['is_guest']) ) {
                                  $_SESSION["captchabox"]=$picture;
                        ?>
                             <p><b><?php echo $lang_common['captchabox post tip']; ?></b></p>
                             <p><input type="image" src="post.php?genImage=true"
                                       alt="Captcha Box"
                                       title="<?php echo $lang_common['captchabox img title'] ?>"
                                       width="<?php echo $picture->imageX; ?>"
                                       height="<?php echo $picture->imageY; ?>"
                                       border="0"></p>
                        <?php } else { ?>
			<p><input type="submit" name="submit" value="<?php echo $lang_common['Submit'] ?>" tabindex="3" accesskey="s" />
                        <?php }; //end else ?>
