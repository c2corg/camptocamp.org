...
else if (isset($_GET['edit_forum']))
{
	...
	if (isset($_POST['save']))
	{
		...
		$db->query('UPDATE '.$db->prefix.'forums SET forum_name=\''.$db->escape($forum_name).'\', forum_desc='.$forum_desc.', redirect_url='.$redirect_url.', sort_by='.$sort_by.', culture=\''.$culture.'\', cat_id='.$cat_id.', parent_forum_id='.$parent_forum_id.' WHERE id='.$forum_id) or error('Unable to update forum', __FILE__, __LINE__, $db->error());

		// Now let's deal with the permissions
		if (isset($_POST['read_forum_old']))
		{
			$result = $db->query('SELECT g_id, g_read_board, g_post_replies, g_post_topics, g_post_polls FROM '.$db->prefix.'groups WHERE g_id!='.PUN_ADMIN) or error('Impossible de retrouver la liste des groupes d\'utilisateurs', __FILE__, __LINE__, $db->error());
			while ($cur_group = $db->fetch_assoc($result))
			{
				...
				if ($read_forum_new != $_POST['read_forum_old'][$cur_group['g_id']] || $post_replies_new != $_POST['post_replies_old'][$cur_group['g_id']] || $post_topics_new != $_POST['post_topics_old'][$cur_group['g_id']] || $post_polls_new != $_POST['post_polls_old'][$cur_group['g_id']])
				{
					...
					else
					{
						...
						$db->query('UPDATE '.$db->prefix.'forum_perms SET read_forum='.$read_forum_new.', post_replies='.$post_replies_new.', post_topics='.$post_topics_new.', post_polls='.$post_polls_new.' WHERE group_id='.$cur_group['g_id'].' AND forum_id='.$forum_id) or error('Impossible d\'insÈrer les permissions de groupes du forum', __FILE__, __LINE__, $db->error());
						if (!$db->affected_rows())
							$db->query('INSERT INTO '.$db->prefix.'forum_perms (group_id, forum_id, read_forum, post_replies, post_topics, post_polls) VALUES('.$cur_group['g_id'].', '.$forum_id.', '.$read_forum_new.', '.$post_replies_new.', '.$post_topics_new.', '.$post_polls_new.')') or error('Impossible d\'insÈrer les permissions de groupes du forum', __FILE__, __LINE__, $db->error());
					}
				}
			}
		}
	...
	// Fetch forum info
	$result = $db->query('SELECT id, forum_name, forum_desc, redirect_url, num_topics, sort_by, culture, cat_id, parent_forum_id FROM '.$db->prefix.'forums WHERE id='.$forum_id) or error('Unable to fetch forum info', __FILE__, __LINE__, $db->error());
	...
	<div class="blockform">
		...
		<div class="box">
			<form id="edit_forum" method="post" action="admin_forums.php?edit_forum=<?php echo $forum_id ?>">
				<p class="submittop"><input type="submit" name="save" value="Save changes" tabindex="7" /></p>
				<div class="inform">
					<fieldset>
...
?>
								...
								<tr>
									...
									<td>
										<select name="culture" tabindex="5">
											<option value="fr"<?php if ($cur_forum['culture'] == 'fr') echo ' selected="selected"' ?>>Fran√ßais</option>
											<option value="en"<?php if ($cur_forum['culture'] == 'en') echo ' selected="selected"' ?>>English</option>
											<option value="it"<?php if ($cur_forum['culture'] == 'it') echo ' selected="selected"' ?>>Italiano</option>
											<option value="de"<?php if ($cur_forum['culture'] == 'de') echo ' selected="selected"' ?>>Deutsch</option>
											<option value="es"<?php if ($cur_forum['culture'] == 'es') echo ' selected="selected"' ?>>Espa√±ol</option>
											<option value="ca"<?php if ($cur_forum['culture'] == 'ca') echo ' selected="selected"' ?>>Catal√†n</option>
								...
								<tr>
									...
									<td><?php echo ($cur_forum['num_topics']) ? 'Only available in empty forums' : '<input type="text" name="redirect_url" size="45" maxlength="100" value="'.pun_htmlspecialchars($cur_forum['redirect_url']).'" tabindex="6" />'; ?></td>
								</tr>
							</table>
						</div>
					</fieldset>
