...
else if (isset($_POST['update']))	// Change position and name of the categories
{
	...
	for ($i = 0; $i < $num_cats; ++$i)
	{
		...
		$db->query('UPDATE '.$db->prefix.'categories SET cat_name=\''.$db->escape($cat_name[$i]).'\', disp_position='.$cat_order[$i].', cat_culture=\''.$cat_culture[$i].'\' WHERE id='.$cat_id) or error('Unable to update category', __FILE__, __LINE__, $db->error());
	}
...
$result = $db->query('SELECT id, cat_name, disp_position, cat_culture FROM '.$db->prefix.'categories ORDER BY disp_position') or error('Unable to fetch category list', __FILE__, __LINE__, $db->error());
$num_cats = $db->num_rows($result);
...
<?php
	...
	for ($i = 0; $i < $num_cats; ++$i)
	{
		list(, list($cat_id, $cat_name, $position, $cat_culture)) = @each($cat_list);

?>
							<tr><td><input type="text" name="cat_name[<?php echo $i ?>]" value="<?php echo pun_htmlspecialchars($cat_name) ?>" size="35" maxlength="80" /></td><td><input type="text" name="cat_order[<?php echo $i ?>]" value="<?php echo $position ?>" size="3" maxlength="3" /></td><td>
                                     <select name="cat_culture[<?php echo $i ?>]" tabindex="5">
				       <option value="fr"<?php if ($cat_culture == 'fr') echo ' selected="selected"' ?>>Français</option>
				       <option value="en"<?php if ($cat_culture == 'en') echo ' selected="selected"' ?>>English</option>
				       <option value="it"<?php if ($cat_culture == 'it') echo ' selected="selected"' ?>>Italiano</option>
