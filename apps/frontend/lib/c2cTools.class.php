...
class c2cTools
{
    ...
    // if you make changes here, be sure also to check varnish vcl file
    public static function mobileRegexp()
    {
        return preg_match('/(Mobile|Symbian|Nokia|SAMSUNG|BlackBerry|Mini|Android)/i', $_SERVER['HTTP_USER_AGENT']);
    }
        ...
        if (in_array($module, array('outings', 'users', 'articles', 'images', 'xreports')))
        {
            ...
            else
            {
                switch ($module)
                {
                    ...
                    case 'xreports': $association_type = 'ux'; break;
