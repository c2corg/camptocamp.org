 ...
 * @version    SVN: $Id: actions.class.php 3782 2007-04-13 17:08:11Z gnat $
 */
...
if (in_array($input_type, array('doctrine_admin_double_list', 'doctrine_admin_check_list', 'doctrine_admin_select_list'))): ?>
      ...
      $<?php echo $name?>Collection = $this-><?php echo $this->getSingularName()?>->get('<?php echo $name ?>');
      $keys = $<?php echo $name?>Collection->getKeys();
      foreach($keys as $key)
      {
        $<?php echo $name?>Collection->remove($key);
      }
...
<?php endforeach; ?>
  ...
  protected function get<?php echo $this->getClassName() ?>OrCreate (<?php echo $this->getMethodParamsForGetOrCreate() ?>)
  {
    ...
    {
      $<?php echo $this->getSingularName() ?> = sfDoctrine::getTable('<?php echo $this->getClassName() ?>')->find(<?php echo $this->getRetrieveByPkParamsForGetOrCreate() ?>);
