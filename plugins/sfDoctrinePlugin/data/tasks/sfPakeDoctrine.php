 ...
 * @version    SVN: $Id: sfPakeDoctrine.php 4581 2007-07-11 21:55:19Z gnat $
 */
...
pake_desc('doctrine build all - generate model and initialize database');
pake_task('doctrine-build-all', 'project_exists');

pake_desc('doctrine build all load - generate model, initialize database, and load data from fixtures');
...
pake_desc('doctrine build database - initialize database');
...
function run_doctrine_insert_sql($task, $args)
{
  ...
  foreach ($directories AS $directory)
  {
    ...
    pake_echo_action("Inserting SQL", $name);
    
    Doctrine::export($directory);
  }
